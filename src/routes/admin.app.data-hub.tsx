...
import {
  adminSyncClientWearables,
  type AdminSyncResult,
} from "@/lib/admin-wearable-sync.functions";
...
function AdminDataHub() {
  ...
  const [visible, setVisible] = useState<Record<SectionKey, boolean>>(() => {
    const base = Object.fromEntries(SECTIONS.map((s) => [s.key, true])) as Record<
      SectionKey,
      boolean
    >;
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(VIS_STORAGE_KEY, JSON.stringify(visible));
    } catch {
      /* ignore */
    }
  }, [visible]);
  ...
  async function runInsight() {
    ...
    try {
      ...
      setInsightText(r.text);
      setInsightAt(r.generatedAt);
      setInsightMemVer(r.memoryVersion ?? 0);
    } catch (e) {
  ...
  const symptomSeries = useMemo(() => {
    ...
    return [...b.checkIns].reverse().map((r: Row) => ({
      ...
      pain: num(r.pain_level),
      sleep: num(r.sleep_quality),
      stress: num(r.stress_level),
      energy: num(r.energy_level),
    }));
  }, [b]);
  const riskSeries = useMemo(() => {
    ...
    return [...b.riskScores]
      .reverse()
      .map((r: Row) => ({ d: shortDate(r.score_date), score: num(r.risk_score) }));
  }, [b]);
  const crossSeries = useMemo(() => {
    if (!b) return [];
    return [...b.loadInsight.crossCheck.days]
      .reverse()
      .map((x) => ({ d: shortDate(x.date), load: x.load, pain: x.pain }));
  }, [b]);
  const vitals = useMemo(() => {
    const mk = (field: string) =>
      b
        ? [...b.wearableSessions]
            .reverse()
            .map((r: Row) => ({ d: shortDate(r.date), v: num(r[field]) }))
            .filter((p) => p.v !== null)
        : [];
    return {
      hrv: mk("hrv_avg"),
      rhr: mk("resting_hr"),
      sleep: mk("sleep_score"),
      steps: mk("total_steps"),
    };
  }, [b]);
  ...
  const predictorBars = useMemo(() => {
    if (!b) return [];
    return b.correlation.predictors.map((p) => ({
      name: `${p.label} (+${p.bestLag}d)`,
      r: p.r,
      abs: Math.abs(p.r),
      dir: p.direction,
    }));
  ...
  const historySeries = useMemo(
    () =>
      b
        ? b.insightHistory.map((h) => ({ d: shortDate(h.date), acwr: h.acwr, fatigue: h.fatigue }))
        : [],
    [b],
  );
  const activitySessions = useMemo(() => {
    ...
    return b.wearableSessions.filter((r: Row) => {
      ...
      return (
        (typeof r.session_type === "string" && r.session_type.trim() !== "") ||
        (dur !== null && dur > 0) ||
        (kcal !== null && kcal > 0) ||
        (load !== null && load > 0)
      );
  ...
  return (
    <div style={{ padding: "20px 16px 24px" }}>
      ...
      >
        ...
        >
          <option value="">
            {loadingList ? "Loading clients…" : `Select a client (${clients.length})`}
          </option>
          {clients.map((c) => (
        ...
        {loadingBundle && (
          <span style={{ color: "var(--white-muted)", fontSize: 12 }}>Loading…</span>
        )}
      </div>

      {err && <div style={{ ...card, borderColor: "var(--red)", color: "var(--red)" }}>{err}</div>}
      {!selected && !err && (
        <div style={{ ...card, color: "var(--white-muted)" }}>
          Choose a client above to see their visual data profile.
        </div>
      )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {visible.overview && (
            <section style={card}>
              ...
              <div style={grid}>
                ...
                <Field
                  label="Yves / AI consent"
                  value={`${b.client.yves_enabled ? "on" : "off"} / ${b.client.yves_ai_consent ? "yes" : "no"}`}
                />
                <Field
                  label="Passive monitoring"
                  value={b.client.passive_monitoring_enabled ? "on" : "off"}
                />
              </div>
            </section>
          )}
          ...
          {visible.insight && (
            <section style={card}>
              <div style={sectionTitle}>
                Yves <span style={countS}>Clinical insight agent</span>
              </div>
              <div style={{ ...muted, fontSize: 11, marginTop: -4, marginBottom: 8 }}>
                Memory v{insightMemVer ?? "—"}
              </div>
              <p style={{ ...muted, marginTop: 0, marginBottom: 12 }}>
                Yves reads all available metrics, symptoms, wearable data and alerts to summarise
              ...
              {insightText ? (
                ...
                <div style={muted}>
                  No insight yet — pick a focus and click <b>Generate insight</b>.
                </div>
              ) : null}
            </section>
          )}
          ...
          {visible.symptoms && (
            <ChartCard
              ...
              empty={symptomSeries.length < 2 ? "Not enough check-ins yet to chart a trend." : null}
            >
              <ChartBox height={240}>
                <LineChart
                  data={symptomSeries}
          ...
          {visible.risk && (
            <ChartCard
              ...
              subtitle="0–100, higher = more concern"
              empty={
                riskSeries.length < 2
                  ? "No risk history yet (needs a wearable baseline + a few check-ins)."
                  : null
              }
            >
              <ChartBox height={200}>
          ...
          {visible.wearable && (
            <section style={card}>
              ...
              {syncErr && <div style={{ ...muted, marginTop: 10, color: C.red }}>{syncErr}</div>}
              {syncMsgs.length > 0 && (
                  ...
                  {syncMsgs.map((m, i) => (
                    <div
                      ...
                      style={{
                        ...
                        background: m.needsClientAction
                          ? "rgba(251,191,36,0.10)"
                          : "rgba(255,255,255,0.04)",
                        color: m.needsClientAction ? C.amber : C.muted,
                        fontSize: 13,
                        lineHeight: 1.45,
                      }}
                    >
                      {m.needsClientAction ? "⚠ " : m.ok ? "✓ " : "• "}
                      {m.message}
                    </div>
                  ))}
                </div>
              ...
              {!b.loadInsight.available ? (
                ...
                <>
                  ...
                  >
                    <Gauge
                      label="ACWR"
                      value={b.loadInsight.metrics.acwr}
                      display={
                        b.loadInsight.metrics.acwr === null
                          ? "—"
                          : b.loadInsight.metrics.acwr.toFixed(2)
                      }
                      pct={
                        b.loadInsight.metrics.acwr === null
                          ? 0
                          : Math.min(b.loadInsight.metrics.acwr / 2, 1) * 100
                      }
                      color={acwrColor(b.loadInsight.metrics.acwr)}
                      note={b.loadInsight.metrics.acwr === null ? "building" : "sweet spot 0.8–1.3"}
                    />
                    <Gauge
                      label="Fatigue"
                      value={b.loadInsight.metrics.fatigueIndex}
                      display={s(b.loadInsight.metrics.fatigueIndex)}
                      pct={b.loadInsight.metrics.fatigueIndex ?? 0}
                      color={bandHigh(b.loadInsight.metrics.fatigueIndex, 80, 70, 50)}
                      note="0–100"
                    />
                    <Gauge
                      label="HRV drop"
                      value={b.loadInsight.metrics.hrvDeviationPct}
                      display={
                        b.loadInsight.metrics.hrvDeviationPct === null
                          ? "—"
                          : `${b.loadInsight.metrics.hrvDeviationPct}%`
                      }
                      pct={
                        b.loadInsight.metrics.hrvDeviationPct === null
                          ? 0
                          : Math.max(0, Math.min(b.loadInsight.metrics.hrvDeviationPct, 100))
                      }
                      color={bandHigh(b.loadInsight.metrics.hrvDeviationPct, 30, 20, 10)}
                      note="vs baseline"
                    />
                    <Gauge
                      label="Sleep"
                      value={b.loadInsight.metrics.recentSleepScore}
                      display={s(b.loadInsight.metrics.recentSleepScore)}
                      pct={b.loadInsight.metrics.recentSleepScore ?? 0}
                      color={bandLow(b.loadInsight.metrics.recentSleepScore, 50, 60, 70)}
                      note="7-day avg"
                    />
                    <Gauge
                      label="Monotony"
                      value={b.loadInsight.metrics.monotony}
                      display={
                        b.loadInsight.metrics.monotony === null
                          ? "—"
                          : String(b.loadInsight.metrics.monotony)
                      }
                      pct={
                        b.loadInsight.metrics.monotony === null
                          ? 0
                          : Math.min(b.loadInsight.metrics.monotony / 2.5, 1) * 100
                      }
                      color={bandHigh(b.loadInsight.metrics.monotony, 2.5, 2.0, 1.5)}
                      note={b.loadInsight.metrics.monotony === null ? "building" : "≥2 elevated"}
                    />
                  </div>
                  {b.loadInsight.metrics.acwr === null && (
                    <div style={{ ...muted, marginTop: 8 }}>
                      Load ratios need 14+ days of wearable history — showing what is available so
                      far.
                    </div>
                  )}
                          ...
                          <Bar dataKey="severity" radius={[0, 4, 4, 0]}>
                            {driverBars.map((d, i) => (
                              <Cell
                                key={i}
                                fill={
                                  d.severity >= 90 ? C.red : d.severity >= 65 ? C.amber : C.blue
                                }
                              />
                            ))}
                          </Bar>
                  ...
                  <div style={{ marginTop: 14 }}>
                    <div style={{ ...fieldLabel, marginBottom: 6 }}>
                      Symptoms vs training load (last 14 days)
                    </div>
          ...
          {visible.predictors && b.wearables.some((w) => w.connected) && (
            <section style={card}>
              <div style={sectionTitle}>
                Symptom predictors
                <span style={countS}>
                  experimental · which health metrics track this client's pain
                </span>
              </div>
              {!b.correlation.available ? (
                ...
                <>
                  {b.correlation.headline && (
                    <div style={{ color: "var(--white)", fontSize: 14, marginBottom: 10 }}>
                      {b.correlation.headline}
                    </div>
                  )}
                  <ChartBox height={Math.max(90, predictorBars.length * 34)}>
                    ...
                    >
                      ...
                      <Tooltip
                        contentStyle={tip}
                        formatter={(v) => [v as number, "correlation r"]}
                      />
                      <Bar dataKey="r" radius={[3, 3, 3, 3]}>
                        {predictorBars.map((p, i) => (
                          <Cell key={i} fill={p.dir === "worse" ? C.red : C.green} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ChartBox>
                  <div style={{ ...muted, fontSize: 11, marginTop: 6 }}>
                    Red = higher metric tracks with more pain · green = with less pain. Positive lag
                    means pain follows the metric by that many days. Association only, not proof of
                    cause — for accuracy testing.
                  </div>
                </>
              )}
            </section>
          ...
          {visible.rhythms && b.wearables.some((w) => w.connected) && (
            <section style={card}>
              ...
              b.rhythms.sleepWeekday === null &&
              b.rhythms.hrvTrend.direction === "unknown" ? (
                <div style={muted}>
                  Not applicable — not enough wearable history to detect rhythms yet.
                </div>
              ) : (
                <>
                  {b.rhythms.notes.length > 0 && (
                  ...
                  <div style={grid}>
                    ...
                    <Field
                      ...
                      value={s(b.rhythms.trainingConsistency.daysActivePerWeek)}
                    />
                    <Field
                      label="Load week-on-week"
                      value={
                        b.rhythms.trainingConsistency.weekOverWeekChangePct === null
                          ? "—"
                          : `${b.rhythms.trainingConsistency.weekOverWeekChangePct}%`
                      }
                    />
                  </div>
                </>
                ...
                {b.patterns.map((p: Row, i) => (
                  <div key={i} style={listRow}>
                    <span style={{ color: "var(--white)" }}>
                      {s(p.metric)} · {s(p.pattern_type)}
                    </span>
                    <span style={{ color: "var(--white-muted)", fontSize: 12 }}>
                      avg {s(p.avg_value)} · conf {s(p.confidence)}
                    </span>
                  </div>
                ))}
              </div>
          ...
          {visible.yves && (
            <section style={card}>
              <div style={sectionTitle}>
                Yves symptom queries <span style={countS}>({b.symptomQueries.length})</span>
              </div>
              {b.symptomQueries.length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {b.symptomQueries.slice(0, 15).map((r: Row, i) => (
                    <div
                      key={i}
                      style={{ ...listRow, flexDirection: "column", alignItems: "stretch", gap: 4 }}
                    >
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                        ...
                        >
                          {s(r.urgency)}
                          {typeof r.severity === "number" ? ` · ${r.severity}/10` : ""}
                          {r.red_flag_detected ? " · 🚩" : ""}
                        </span>
                      </div>
                      {r.ai_rationale ? (
          ...
          {visible.alerts && (
            <section style={card}>
              <div style={sectionTitle}>
                Alerts <span style={countS}>({b.alerts.length})</span>
              </div>
              {b.alerts.length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {b.alerts.slice(0, 20).map((r: Row, i) => (
                    <div key={i} style={listRow}>
                      ...
                      >
                        <span
                          style={{ color: urgencyColor(r.urgency), fontSize: 12, fontWeight: 600 }}
                        >
                          {s(r.urgency)}
                        </span>
...
function ChartCard({
  ...
  return (
    <section style={card}>
      <div style={sectionTitle}>
        {title}
        {subtitle ? <span style={countS}>{subtitle}</span> : null}
      </div>
  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
        ...
        <span style={{ color: "var(--white)", fontWeight: 700, fontSize: 15 }}>
          {latest === null ? "—" : latest}
        </span>
      </div>
      {data.length === 0 ? (
        <div style={{ ...muted, fontSize: 11, marginTop: 8, fontStyle: "italic" }}>
          No data received yet — Garmin/Oura only report this after a synced night, and it can take
          a day or two after connecting to arrive.
        </div>
      ) : data.length < 2 ? (
        <div style={{ ...muted, fontSize: 11, marginTop: 8 }}>
          Only one reading so far — building history.
        </div>
      ) : (
      ...
      <div style={{ position: "relative", width: size, height: size }}>
        <svg width={size} height={size}>
          ...
          <circle
            ...
            cy={size / 2}
            r={r}
            stroke={value === null ? "var(--navy-border)" : color}
            strokeWidth={stroke}
            fill="none"
            strokeLinecap="round"
            strokeDasharray={c}
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div style={{ color: "var(--white)", fontSize: 14 }}>{value}</div>
    </div>
  );
}
