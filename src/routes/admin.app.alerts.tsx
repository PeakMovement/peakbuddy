...
import type { Alert, Client, Profile } from "@/lib/types";

export const Route = createFileRoute("/admin/app/alerts")({
  head: () => ({ meta: [{ title: "All Alerts — Buddy Admin" }] }),
  component: AllAlerts,
});

type Filter = "all" | "unread" | "emergency" | "urgent";
...
function AllAlerts() {
  ...
  useEffect(() => {
    (async () => {
      ...
      ((c as Client[]) ?? []).forEach((x) => (cm[x.id] = x));
      setClients(cm);
      const pm: Record<string, Profile> = {};
      ((p as Profile[]) ?? []).forEach((x) => (pm[x.id] = x));
      setProfiles(pm);
      setLoading(false);
    })();
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "emergency") return sorted.filter((a) => a.urgency === "emergency");
    if (filter === "urgent") return sorted.filter((a) => a.urgency === "urgent");
    return sorted;
  }, [alerts, filter]);
  ...
  const filters: Filter[] = ["all", "unread", "emergency", "urgent"];
      ...
      {loading ? (
        ...
        <div style={{ marginTop: 16, display: "flex", flexDirection: "column", gap: 10 }}>
          {filtered.map((a) => {
            ...
            const prac = profiles[a.practitioner_id];
            return (
              <div
                key={a.id}
                style={{
