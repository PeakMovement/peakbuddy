  ...
  return (
    <div style={{ padding: "16px 16px 32px" }}>
      ...
      <Card>
        ...
        <AIFeaturesRow practice={practice} onChange={(p) => setPractice(p)} />
        <GamificationRow practice={practice} onChange={(p) => setPractice(p)} />
      </Card>
...
function GamificationRow({
  ...
  const enabled = practice?.gamification_enabled !== false;
  const toggle = async () => {
    ...
    const { data, error } = await supabase
      .from("practices")
      .update({ gamification_enabled: next })
    ...
    if (error) {
      ...
      onChange({
        ...practice,
        gamification_enabled: (data as { gamification_enabled: boolean }).gamification_enabled,
      });
    }
    setSaving(false);
  };
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
      <div style={{ minWidth: 0 }}>
        ...
        >
          When off, this practitioner's clients see no streaks and earn no rewards.
        </div>
      </div>
      <button
        type="button"
...
function AIFeaturesRow({
  ...
  const [saving, setSaving] = useState(false);
  const [err, setErr] = useState<string | null>(null);
  const enabled = practice?.ai_features_enabled === true;
  const toggle = async () => {
    if (!practice || saving) return;
    setSaving(true);
    setErr(null);
    ...
    const { data, error } = await supabase
      .from("practices")
      .update({
        ai_features_enabled: next,
        // Keep legacy flags in sync so older code paths stay consistent
        yves_enabled: next,
        programs_suggest_enabled: next,
      })
      .eq("practitioner_id", practice.practitioner_id)
      .select("ai_features_enabled, yves_enabled, programs_suggest_enabled")
      .maybeSingle();
    if (error) {
      setErr(error.message);
    } else if (!data) {
      ...
      onChange({
        ...practice,
        ai_features_enabled: (data as { ai_features_enabled: boolean }).ai_features_enabled,
        yves_enabled: (data as { yves_enabled: boolean }).yves_enabled,
        programs_suggest_enabled: (data as { programs_suggest_enabled: boolean })
          .programs_suggest_enabled,
      });
    }
