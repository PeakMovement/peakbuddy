...
export function DetectionSettingsPanel() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setSt(d);
        setTh(d.thresholds);
        setReport(d.suggestions ?? null);
      } catch {
        /* not super admin / not ready */
      }
    })();
  }, [load]);
  ...
  const saveAlerting = async (patch: Record<string, unknown>) => {
    setBusy("al");
    try {
      await saveAlert({ data: patch });
      setSt({ ...st, ...mapPatch(st, patch) });
      flash("Saved.");
    } catch (e) {
      flash(e instanceof Error ? e.message : "Save failed");
    } finally {
  ...
  const escalate = async () => {
    setBusy("esc");
    try {
      const r = await runEsc();
      flash(r.skipped ? "Escalation is off — nothing sent." : `Escalated ${r.escalated} alert(s).`);
    } catch (e) {
      flash(e instanceof Error ? e.message : "Failed");
    } finally {
      setBusy(null);
  ...
  const pairs: [keyof Thresholds, string][] = [
    ["acwr", "ACWR"],
    ["monotony", "Monotony"],
    ["strain", "Strain"],
    ["fatigue", "Fatigue"],
  ...
  return (
    <section style={{ marginTop: 32 }}>
      ...
      <p style={sub}>
        Tune the load/injury thresholds used on the Data Hub, review calibration from practitioner
        feedback, and control escalation. These do not change the client app.
      </p>
      ...
      <div style={card}>
            ...
            <div key={m}>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                <input
                  ...
                  type="number"
                  step="0.1"
                  value={(th[m] as Record<string, number>).elevated}
                  onChange={(e) =>
                    edit(
                      m,
                      "elevated",
                      NUM(e.target.value, (th[m] as Record<string, number>).elevated),
                    )
                  }
                />
                <input
                  ...
                  type="number"
                  step="0.1"
                  value={(th[m] as Record<string, number>).critical}
                  onChange={(e) =>
                    edit(
                      m,
                      "critical",
                      NUM(e.target.value, (th[m] as Record<string, number>).critical),
                    )
                  }
                />
              </div>
        ...
        <button style={primary} disabled={busy === "th"} onClick={saveThresholds}>
          {busy === "th" ? "Saving…" : "Save thresholds"}
        </button>
      </div>
      ...
      <div style={card}>
        ...
        <p style={sub}>
          Reads confirmed / false-alarm outcomes on alerts and reports per-category precision, so
          you can see which way to tune. Applying a change stays your decision.
        </p>
        <button style={ghost} disabled={busy === "cal"} onClick={calibrate}>
          {busy === "cal" ? "Running…" : "Run calibration now"}
        </button>
        {st.calibratedAt && (
          <span style={{ ...muted, marginLeft: 10 }}>
            last run {new Date(st.calibratedAt).toLocaleString()}
          </span>
        )}
            ...
            {report.categories.map((c) => (
              <div key={c.category} style={row}>
                ...
                <span style={{ color: "var(--white-muted)", fontSize: 12 }}>
                  {c.confirmed}/{c.n} real · {(c.precision * 100).toFixed(0)}% ·{" "}
                  <b
                    style={{
                      color:
                        c.suggestion === "raise"
        ...
        {report && report.categories.length === 0 && (
          <div style={{ ...muted, marginTop: 8 }}>No graded alerts yet.</div>
        )}
      </div>
      ...
      <div style={card}>
        ...
        <label style={toggle}>
          ...
          <input
            type="checkbox"
            checked={st.escalation.enabled}
            onChange={(e) => saveAlerting({ escalationEnabled: e.target.checked })}
            style={{ width: 22, height: 22, accentColor: "var(--blue-accent)" }}
          />
        </label>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          <div>
            <div style={lbl}>After (minutes)</div>
            <input
              style={inp}
              type="number"
              defaultValue={st.escalation.afterMinutes}
              onBlur={(e) =>
                saveAlerting({
                  escalationAfterMinutes: NUM(e.target.value, st.escalation.afterMinutes),
                })
              }
            />
          </div>
          <div>
            <div style={lbl}>Min urgency</div>
            <select
              style={{ ...inp, minWidth: 120 }}
              value={st.escalation.minUrgency}
              onChange={(e) => saveAlerting({ escalationMinUrgency: e.target.value })}
            >
              {["soon", "urgent", "emergency"].map((u) => (
        ...
        <button style={ghost} disabled={busy === "esc"} onClick={escalate}>
          {busy === "esc" ? "Running…" : "Run escalation sweep now"}
        </button>
...
function mapPatch(
  ...
  if (patch.escalationAfterMinutes !== undefined)
    esc.afterMinutes = patch.escalationAfterMinutes as number;
  if (patch.escalationMinUrgency !== undefined)
    esc.minUrgency = patch.escalationMinUrgency as string;
  return {
    escalation: esc,
    autoCalibrate:
      patch.autoCalibrate !== undefined ? (patch.autoCalibrate as boolean) : st.autoCalibrate,
  };
}
