...
describe("buildCorrelation", () => {
  ...
  it("finds load predicting pain with a 2-day lag", () => {
    ...
    for (let a = 2; a < vals.length; a++)
      checkIns.push({ created_at: iso(a - 2), pain_level: Math.min(10, vals[a]) });
  ...
  it("does not fabricate a predictor from a constant metric", () => {
    const sessions: WearableDay[] = Array.from({ length: 14 }, (_, a) => ({
      date: iso(a),
      training_load: 100,
      hrv_avg: 50,
    }));
    const checkIns: CheckInDay[] = Array.from({ length: 10 }, (_, a) => ({
      created_at: iso(a),
      pain_level: (a % 5) + 2,
    }));
    const r = buildCorrelation(sessions, checkIns, true);
