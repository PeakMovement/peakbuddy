...
export const askYves = createServerFn({ method: "POST" })
  ...
  .handler(
    async ({
      ...
      const memoryRules = (memRes.data ?? []) as Array<{
        scope: string;
        rule_type: string;
        title: string;
        rule_text: string;
      ...
      const memoryVersion =
        (verRes.data as { version_number?: number } | null)?.version_number ?? 0;
      ...
      if (!res.ok) {
        ...
        if (res.status === 429)
          throw new Error("Yves is busy right now — please try again in a minute.");
        if (res.status === 402)
    ...
    async ({
      ...
      const [pubRes, stgRes, verRes] = await Promise.all([
        ...
        db
          ...
          .select(
            "id, scope, rule_type, title, rule_text, rationale, status, conflict_flags, created_at",
          )
          .order("created_at", { ascending: false })
          .limit(100),
        db
      ...
      return {
        ...
        staging: ((stgRes.data ?? []) as Array<Record<string, unknown>>).map((r) => ({
          ...
          conflict_flags: Array.isArray(r.conflict_flags)
            ? (r.conflict_flags as unknown[]).map(String)
            : [],
...
function regexSanitise(text: string): string | null {
  ...
  if (/\b\d{6,}\b/.test(text))
    return "Contains a long numeric sequence (possible ID or record number).";
  // South African ID-number-like tokens (13 digits, possibly spaced).
  if (/\b\d{2}\s?\d{2}\s?\d{2}\s?\d{4}\s?\d{3}\b/.test(text))
    return "Contains an ID-number-like token.";
    ...
    async ({
      ...
      try {
        ...
        draft = {
          title: String(parsed.title ?? "")
            .slice(0, 80)
            .trim(),
          rule_type: (RULE_TYPES as readonly string[]).includes(rt)
            ? (rt as RuleType)
            : "reasoning",
          scope: normalizeScope(String(parsed.scope ?? targetScope).trim(), targetScope),
          rule_text: String(parsed.rule_text ?? "")
            .slice(0, 600)
            .trim(),
        ...
        await logAttempt(
          ...
          data.feedbackId,
          data.correction,
          "Model did not return a usable rule draft.",
          null,
        );
        return {
          ok: false,
          reason:
            "Yves could not draft a reusable rule from that correction. Please rephrase it as a general clinical principle.",
        };
      }
      ...
      if (regexFail) {
        await logAttempt(
          ...
          data.feedbackId,
          data.correction,
          `Regex sanitiser blocked: ${regexFail}`,
          draft,
        );
        return {
          ok: false,
      ...
      if (identifiable) {
        await logAttempt(
          ...
          data.feedbackId,
          data.correction,
          `Classifier blocked: ${classReason}`,
          draft,
        );
        return {
          ok: false,
      ...
      const active = (activeRes.data ?? []) as Array<{
        id: string;
        title: string;
        rule_text: string;
      }>;
