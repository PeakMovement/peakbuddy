...
function trend(vals: { key: string; v: number }[]): Trend {
  ...
  if (recent === null || older === null)
    return { recent: round(recent), older: round(older), direction: "unknown" };
...
export function buildRhythms(sessions: WearableDay[]): RhythmPatterns {
  ...
  for (const s of sleep) {
    const dow = new Date(s.key + "T00:00:00Z").getUTCDay();
    (dow === 0 || dow === 6 ? weekend : weekday).push(s.v);
  }
  ...
  if (sleepWeekday !== null && sleepWeekend !== null) {
    ...
    if (Math.abs(diff) >= 5)
      notes.push(
        `Sleep score is ~${Math.abs(diff)} pts ${diff > 0 ? "lower on weekends" : "lower on weekdays"}.`,
      );
  }

  const hrvTrend = trend(latestPerDay(sessions, "hrv_avg"));
  if (hrvTrend.direction === "down")
    notes.push("HRV is trending down over the last two weeks — reduced recovery.");
  ...
  if (rhrTrend.direction === "up")
    notes.push("Resting heart rate is drifting up — possible accumulating fatigue.");
  ...
  if (method)
    for (const s of sessions) {
      const k = dayKey(s.date);
      if (!loadDays.has(k)) {
        const l = loadForDay(s, method);
        if (l !== null && l > 0) loadDays.set(k, l);
      }
    }
  ...
  const weekOverWeekChangePct =
    l7 !== null && p7 !== null && p7 > 0 ? Math.round(((l7 - p7) / p7) * 100) : null;
  if (weekOverWeekChangePct !== null && Math.abs(weekOverWeekChangePct) >= 30)
    notes.push(
      `Weekly training load ${weekOverWeekChangePct > 0 ? "up" : "down"} ${Math.abs(weekOverWeekChangePct)}% vs the week before.`,
    );
  ...
  return {
    ...
    hrvTrend,
    rhrTrend,
    trainingConsistency: { daysActivePerWeek, weekOverWeekChangePct },
    notes,
  };
}
