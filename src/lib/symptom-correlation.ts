...
const addDays = (key: string, d: number) => {
...
export function buildCorrelation(
  ...
  const none = (reason: string): CorrelationResult => ({
    available: false,
    reason,
    predictors: [],
    headline: null,
  ...
  if (method)
    for (const sfx of sessions) {
      const k = dayKey(sfx.date);
      if (!loadByDate.has(k)) {
        const l = loadForDay(sfx, method);
        if (l !== null) loadByDate.set(k, l);
      }
    }
  ...
  for (const m of metricDefs) {
    ...
    const strength =
      Math.abs(best.r) >= 0.6 ? "a strong" : Math.abs(best.r) >= 0.4 ? "a moderate" : "a weak";
    const lagTxt =
    ...
    predictors.push({
      ...
      n: best.n,
      confidence: Math.min(1, best.n / 14),
      direction,
      sentence,
    });
  }
  ...
  if (predictors.length === 0)
    return none(
      "No clear symptom↔metric association yet — needs more overlapping wearable and check-in days.",
    );
