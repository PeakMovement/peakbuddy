...
async function sanitiseOrThrow(
  key: string,
  title: string,
  ruleText: string,
  ...
  if (regexFail)
    throw new Error(`Blocked by privacy check: ${regexFail} Please generalise the rule.`);
...
export const publishYvesRule = createServerFn({ method: "POST" })
  ...
  .handler(
    async ({
      ...
      if (stg.status !== "pending")
        throw new Error(`Cannot approve a rule with status "${stg.status}".`);
...
export const rollbackYvesMemory = createServerFn({ method: "POST" })
  ...
  .handler(
    async ({ data, context }): Promise<{ ok: true; newVersion: number; restoredCount: number }> => {
      await assertSuperAdmin(context.supabase, context.userId);
      const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
      const db = supabaseAdmin as unknown as SupabaseClient;
      ...
      const deact = await db.from("yves_memory").update({ is_active: false }).eq("is_active", true);
      if (deact.error) throw new Error(deact.error.message);
