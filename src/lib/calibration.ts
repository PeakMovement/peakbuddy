...
export function computeCalibration(rows: AlertOutcomeRow[], now = new Date()): CalibrationReport {
  ...
  for (const [category, c] of byCat) {
    ...
    if (n >= MIN_SAMPLE && precision < LOW_PRECISION) {
      suggestion = "raise";
      rationale = `Too many false alarms (${c.falseAlarm}/${n}) — raise the threshold to reduce noise.`;
    } else if (n >= MIN_SAMPLE && precision >= HIGH_PRECISION) {
      suggestion = "lower";
      rationale = `High precision (${c.confirmed}/${n} real) — could safely lower the threshold to catch more.`;
